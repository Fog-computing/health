...
public final class UploadUtil {
    ...
    private static final String appUploadFileDir = "/WEB-INF/app/upload";
    /**
     * 上传文件缓存大小限制
     */
     ...
     * 上传文件处理(支持批量)
     */
    public static List<String> uploadImage(HttpServletRequest request, boolean lessen, boolean app) {
        CommonsMultipartResolver multipartResolver = new CommonsMultipartResolver (
                request.getSession ().getServletContext ());
        List<String> fileNames = InstanceUtil.newArrayList ();
        if (multipartResolver.isMultipart (request)) {
            ...
            String pathDir;
            if (app) pathDir = getAppUploaddDir (request);
            else pathDir = getUploadDir (request);
            File dirFile = new File (pathDir);
            if (!dirFile.isDirectory ()) {
                dirFile.mkdirs ();
            }
    ...
    public static String getAppUploaddDir(HttpServletRequest request) {
        return request.getServletContext ().getRealPath (appUploadFileDir) + File.separator;
    }
